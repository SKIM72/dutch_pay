...
test('영수증 사진은 서버 전송 없이 검토 후 기존 지출 입력칸에 적용된다', async ({ page }) => {
  ...
  await expect(page.locator('.receipt-privacy-note')).toContainText('서버나 데이터베이스로 전송되지 않습니다');
  ...
  await expect(page.locator('#receipt-result-name')).toHaveValue('영수증 스캔 항목');
  ...
  await expect(page.locator('#item-name')).toHaveValue('영수증 스캔 항목');
  await expect(page.locator('#item-date')).not.toHaveValue('');
