...
test('영수증 사진은 OCR 분석 후 검토를 거쳐 기존 지출 입력칸에 적용된다', async ({ page }) => {
  ...
  await expect(page.locator('#receipt-amount-evidence')).toContainText('결제금액 32,800원');
  ...
  expect(calls.some((call) => (
    ...
    && call.body.hasImage
    && call.body.hasEnhancedImage
    && call.body.mimeType === 'image/jpeg'
    && /^\d{4}-\d{2}-\d{2}$/.test(call.body.currentLocalDate)
...
test('배경이 포함된 영수증 사진은 자동으로 모서리를 찾아 원근 보정한다', async ({ page }) => {
  const result = await page.evaluate(async () => {
    ...
    return {
      ...
      detectionConfidence: prepared.detectionConfidence,
      hasEnhancedImage: prepared.enhancedBlob instanceof Blob,
  ...
  expect(result.hasEnhancedImage).toBe(true);
...
test('작은 밝은 물체는 영수증으로 오인해 자동 자르지 않는다', async ({ page }) => {
  const result = await page.evaluate(async () => {
    const canvas = document.createElement('canvas');
    canvas.width = 1200;
    canvas.height = 1600;
    ...
    context.fillRect(820, 180, 230, 280);
    context.fillStyle = '#1f2937';
    context.font = '26px sans-serif';
    context.fillText('NOT A RECEIPT', 835, 250);

    const blob = await new Promise((resolve) => canvas.toBlob(resolve, 'image/jpeg', 0.9));
    const file = new File([blob], 'small-bright-object.jpg', { type: 'image/jpeg' });
    const prepared = await window.SettleUpReceiptImage.prepare(file);
    return {
      autoCropped: prepared.autoCropped,
      detectionConfidence: prepared.detectionConfidence
  ...
  expect(result.detectionConfidence).toBeLessThan(0.7);
});
